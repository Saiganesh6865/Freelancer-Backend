...
def signup_user(total_users, data):
    ...
    if total_users == 0:
        ...
        user, error = create_user(username, email, password, role, manager_type)
        if error:
...
def create_user(username, email, password, role, manager_type=None):
    ...
    # Assign manager_type if role is manager
