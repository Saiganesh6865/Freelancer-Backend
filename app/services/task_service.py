...
def edit_task(manager_id, task_id, data):
    ...
    # Coerce to ints when possible to avoid "2" != 2 problems
    try:
        job_manager_id = int(job.manager_id)
    except (TypeError, ValueError):
        job_manager_id = job.manager_id

    try:
        req_manager_id = int(manager_id)
    except (TypeError, ValueError):
        req_manager_id = manager_id

    if job_manager_id != req_manager_id:
        raise PermissionError("Unauthorized")
