...
def get_manager_freelancers(manager_id):
    ...
    # Get all job IDs managed by this manager
    job_ids = [job.id for job in Job.query.filter_by(manager_id=manager_id).all()]

    # Get all freelancers who have applied or are in any batch for these jobs
    # But if you want to list literally all freelancers, skip this filtering
    freelancers = User.query.filter_by(role="freelancer").all()
...
from app.repositories.batch_repository import assign_freelancers_to_batch, get_batch_members
...
def get_manager_batches(manager_id):
    ...
    for batch in batches:
        # Use relationship to get members (BatchMember table)
        batch_member = BatchMember.query.filter_by(batch_id=batch.id).first()
        members = batch_member.team_members.split(",") if batch_member and batch_member.team_members else []
