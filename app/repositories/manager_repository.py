# app/repositories/manager_repository.py
...
def fetch_dashboard_metrics(manager_id):
    ...
    total_jobs = db.session.query(func.count(Job.id)).filter_by(manager_id=manager_id).scalar()
    total_applications = db.session.query(func.count(Application.id)).join(Job).filter(Job.manager_id == manager_id).scalar()
